...
namespace HexGlobeProject.Tests.Editor
{
    public class IcosphereMapping_BaryLocalToGlobalTests
    {
        ...
        public void BaryLocalToGlobal_Reflection_Behavior_IsStableOnBoundary()
        {
            ...
            var g = IcosphereMapping.BaryLocalToGlobal(id, new(local[0], local[1]), res);
            ...
            Assert.AreEqual(1f, g.U + g.V, 1e-4f, "Boundary sum should be ~1 when not reflected");

            // Now test a value slightly over 1
            var localOver = new Barycentric(local[0] + 0.001f, local[1] + 0.001f);
